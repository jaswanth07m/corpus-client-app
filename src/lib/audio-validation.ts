...
export async function validateAudioFile(file: File): Promise<AudioValidationResult> {
  ...
  // Check file format
  const validAudioFormats = ['audio/wav', 'audio/mpeg', 'audio/mp3', 'audio/ogg', 'audio/webm', 'audio/flac'];
  ...
  try {
    ...
    try {
      ...
      } else if (durationSeconds > 900) { // 15 minutes = 900 seconds
...
function checkForSilence(audioBuffer: AudioBuffer): boolean {
  ...
  for (let i = 0; i < sampleSize; i++) {
    const sample = channelData[Math.floor(i * (channelData.length / sampleSize))];
